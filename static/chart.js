...
function computeSMA10(){
    ...
    for(let i=9;i<candles.length;i++){
        ...
        sma10.push({t:candles[i].t, v:sum/10});
    }
}
...
function updateInfo(){
    ...
    let canalElem=document.getElementById('canalInfo');
    if(canalElem)canalElem.innerHTML='<span id="canalLabel"></span>: '+canal;
    let sr='';
    if(lastR)sr+='R: '+lastR.price.toFixed(dec);
    if(lastS)sr+=(sr?' | ':'')+'S: '+lastS.price.toFixed(dec);
    let srElem=document.getElementById('srInfo');
    if(srElem)srElem.innerHTML='<span id="srLabel"></span>: '+(sr||'--');
    let sig='';
    if(divergences.length>0)sig=divergences[0].type;
    else if(patterns.length>0)sig=patterns[0].type;
    else sig='Aucun';
    let sigElem=document.getElementById('signalInfo');
    if(sigElem)sigElem.innerHTML='<span id="signalsLabel"></span>: '+sig;
    let stats='';
    ...
    stats+=' | '+n+' bougies';
    let statsElem=document.getElementById('statsInfo');
    if(statsElem)statsElem.innerHTML='<span id="statsLabel"></span>: '+(stats||'--');
}
...
function loop(){
    ...
    if(sma10.length>0){
        ...
        ctx.setLineDash([]);
        let first=true;
        for(let i=0;i<sma10.length;i++){
            let idx = 9 + i; // l'index dans candles
            if(idx>=si && idx<n){
                let x=L+(idx-si)*tw+2;
                let y=Y(sma10[i].v);
                if(first){
