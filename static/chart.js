var canvas,ctx,candles=[],price=0,ws=null,sym='R_75',tf=5,cw=8;
...
function resize(){canvas.width=window.innerWidth-340;canvas.height=window.innerHeight-120;}

function getGranularity(){
    switch(parseInt(document.getElementById('tf').value)){
        case 5:return 300;case 15:return 900;case 60:return 3600;case 240:return 14400;default:return 300;
    }
}
function getUpdateInterval(){
    switch(parseInt(document.getElementById('tf').value)){
        case 5:return 10000;case 15:return 30000;case 60:return 60000;case 240:return 240000;default:return 30000;
    }
}

function computeSMA10(){
    sma10=[];if(candles.length<10)return;
    for(let i=9;i<candles.length;i++){let s=0;for(let j=i-9;j<=i;j++)s+=candles[j].c;sma10.push({t:candles[i].t,v:s/10});}
}
...
function connect(){
    ...
    patterns=[];divergences=[];trendLines=[];pivotLevels=null;lastDivLine=null;sma10=[];
    
    function fetchCandles(){
        ws=new WebSocket('wss://ws.binaryws.com/websockets/v3?app_id=1089');
        ws.onopen=()=>{
            ws.send(JSON.stringify({ticks_history:sym,count:500,end:'latest',start:1,style:'candles',granularity:getGranularity()}));
        };
        ws.onmessage=(m)=>{
            ...
            if(d.candles){
                ...
                if(candles.length>0){price=candles[candles.length-1].c;document.getElementById('price').innerText=price.toFixed(sym.includes('frx')?5:2);}
    ...
    fetchCandles();
    updateInterval=setInterval(fetchCandles,getUpdateInterval());
}

function findLastSR(){
    lastR=null;lastS=null;if(candles.length<10)return;let n=candles.length;
...
function computePivots(){
    if(candles.length<24)return;let prev=candles[Math.max(0,candles.length-96)],H=prev.h,C=prev.c,L=prev.l;
    let PP=(H+C+L)/3;pivotLevels={PP:PP,R1:2*PP-L,R2:PP+(H-L),S1:2*PP-H,S2:PP-(H-L)};
}
function computeTrendLines(){
    trendLines=[];if(candles.length<20)return;let n=candles.length,highs=[],lows=[];
...
function computeMACD(){
    if(candles.length<35)return;let closes=candles.map(c=>c.c);
...
function detectPatterns(){
    patterns=[];if(candles.length<2)return;let a=candles[candles.length-1],body=Math.abs(a.c-a.o),avgBody=0;
    for(let i=Math.max(0,candles.length-20);i<candles.length;i++)avgBody+=Math.abs(candles[i].c-candles[i].o);
    avgBody/=20;if(body>avgBody*2.5)patterns.push(a.c>a.o?{type:'LONGUE VERTE',color:'#3fb950'}:{type:'LONGUE ROUGE',color:'#f85149'});
}
function detectDivergences(){
    divergences=[];lastDivLine=null;if(macdData.length<40)return;let n=candles.length;
    ...
    if(pH1>pH2&&macdData[pH1i]&&macdData[pH2i]&&macdData[pH1i].v<macdData[pH2i].v){divergences.push({type:'DIVERGENCE BAISSIERE',color:'#f85149'});lastDivLine={i1:pH2i,i2:pH1i,color:'#f85149',price:true};}
    ...
    if(pL1<pL2&&macdData[pL1i]&&macdData[pL2i]&&macdData[pL1i].v>macdData[pL2i].v){divergences.push({type:'DIVERGENCE HAUSSIERE',color:'#3fb950'});lastDivLine={i1:pL2i,i2:pL1i,color:'#3fb950',price:true};}
}
function updateInfo(){
    let n=candles.length,dec=sym.includes('frx')?5:1,canal='--';
    if(n>=20){let sx=0,sy=0,sxy=0,sx2=0;for(let i=0;i<n;i++){sx+=i;sy+=candles[i].c;sxy+=i*candles[i].c;sx2+=i*i;}let slope=(n*sxy-sx*sy)/(n*sx2-sx*sx);canal=slope>0?'HAUSSIER':slope<0?'BAISSIER':'NEUTRE';}
    document.getElementById('canalVal').innerText=canal;
    let sr='';if(lastR)sr+='R:'+lastR.price.toFixed(dec);if(lastS)sr+=(sr?' | ':'')+'S:'+lastS.price.toFixed(dec);
    document.getElementById('srVal').innerText=sr||'--';
    let sig='';if(divergences.length>0)sig=divergences[0].type;else if(patterns.length>0)sig=patterns[0].type;else sig='Aucun';
    document.getElementById('signalVal').innerText=sig;
    let stats='';if(pivotLevels)stats+='PP:'+pivotLevels.PP.toFixed(dec);stats+=' | '+n+' bougies';
    document.getElementById('statsVal').innerText=stats||'--';
}
...
function loop(){
    ...
    if(n>=20){
        let sx=0,sy=0,sxy=0,sx2=0;for(let i=0;i<n;i++){sx+=i;sy+=candles[i].c;sxy+=i*candles[i].c;sx2+=i*i;}
